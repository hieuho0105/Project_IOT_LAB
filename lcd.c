...
void lcd_tx_mode_1_line(const struct sl_memlcd_t *device, const void *data, unsigned int row_start, unsigned int row_count) {
  ...
  for (row = row_start; row < row_start + row_count; row++) {
    ...
    sli_memlcd_spi_tx(&spi_handle, &dummy, 2); // Gửi 16 byte dữ liệu cho dòng (độ rộng 128 pixel = 16 byte)
    sli_memlcd_spi_wait(&spi_handle);
...
sl_status_t lcd_tx_mode_multiple_line(const struct sl_memlcd_t *device, const void *data, unsigned int row_start, unsigned int row_count) {
    uint16_t cmd, dummy;
    const uint8_t *data_ptr = (const uint8_t *)data;
    unsigned int row;
    ...
    sli_memlcd_spi_tx(&spi_handle, &dummy, sizeof(dummy));
    sli_memlcd_spi_wait(&spi_handle);
...
sl_status_t lcd_tx(const struct sl_memlcd_t *device, const void *data, unsigned int row_start, unsigned int row_count) {
  ...
  lcd_tx_mode_1_line(device, data, row_start, row_count);

  //lcd_tx_mode_multiple_line(device, data, row_start, row_count);
